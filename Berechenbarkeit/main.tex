...
\mysubsection{Bemerkung(Unendlich viele Indizes)*} 
...
\mysubsection{Bemerkung(Universelle Turingmaschine U)*} 
...
\mysubsection{Proposition(Diagonales Halteproblem)*} 
...
\mysubsection{Satz(Diagonalhalteproblem - Nicht entscheidbar)*} 
...
\mysubsection{Bemerkung(Entscheidbarkeit unter Reduzibilität)*} 
  \begin{itemize}
...
\mysubsection{Satz(Nicht-Entscheidbarkeit des initialen Halteproblems)*} 
...
\mysubsection{Lemma(PCP-Reduktion: \(\Gamma \to \Sigma\))*} 
  ...
  \mysubsection{Lemma(Reduktion von MPCP über \(\Sigma\) auf PCP über \(\Sigma\))*} 
  ...
  \mysubsection{Lemma(Reduktion von \(H_{init}\) auf \(MPCP_{\Box, 0, 1, *, 6, +}\))*}
  ...
  \mysubsection{Beispiel(TM \(M_e\) mit \(e \in \mathbb{N}_0\)))*} 
  ...
  \mysubsubsection{Definition (Fixpunktsatz)} 
